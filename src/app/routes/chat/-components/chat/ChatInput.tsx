...
import {
  ImagePreview,
  type ImageSlide,
} from "@/app/components/ui/image-preview";
...
export function ChatInput({
  ...
  const { currentModelCapability, maxImages } = (() => {
    ...
    if (!currentModelAbility)
      return { currentModelCapability: null, maxImages: 1 };

    return {
      currentModelCapability: currentModelAbility,
      maxImages:
        currentModelAbility === "i2i" ? currentModelMaxInputImages || 1 : 1,
    };
  })();

  // Determine upload restrictions based on model capability
  const canUploadImages =
    currentModelCapability && currentModelCapability !== "t2i";
