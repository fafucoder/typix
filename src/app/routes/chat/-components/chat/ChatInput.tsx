...
interface ChatInputProps {
	...
	currentModel?: string;
	currentModelAbility?: Ability | null;
	currentModelMaxInputImages?: number;
...
export function ChatInput({ 
	...
	const { currentModelCapability, maxImages } = (() => {
		...
		if (!currentModelAbility) return { currentModelCapability: null, maxImages: 1 };

		return {
			currentModelCapability: currentModelAbility,
			maxImages: currentModelAbility === "i2i" ? (currentModelMaxInputImages || 1) : 1,
		};
	})();
				...
				/>
						...
						<div className="absolute inset-x-3 bottom-3 flex items-center justify-between">
							...
							{showPreferences && (
								...
								{/* Upload button - only show for i2i models */}
								{canUploadImages && (
									<TooltipProvider>
										<Tooltip>
											<TooltipTrigger asChild>
												<Button
													...
													className="h-8 w-8 rounded-lg border-border/50 bg-background/80 backdrop-blur-sm transition-all duration-200 hover:scale-105 hover:bg-accent/80 disabled:opacity-50"
												>
											...
											<TooltipContent>
												<p>
													{selectedImages.length >= maxImages
														? t("chat.maxImagesReached", { max: maxImages })
														: t("chat.uploadImage")}
												</p>
											</TooltipContent>
										</Tooltip>
							...
							<Button
								...
								className="h-8 gap-2 rounded-lg px-4 transition-all duration-200 hover:scale-105"
							>
